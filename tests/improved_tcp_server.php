...
function handleClient(TCPStream $client): Generator
{
    ...
    try {
        // Set timeouts for this client
        $client->setReadTimeout(60);  // 60 seconds read timeout
        $client->setWriteTimeout(30); // 30 seconds write timeout
        $client->setBufferSize(4096); // 4KB buffer size
        ...
        while (! $client->isClosed()) {
            ...
            $response .= "Content-Length: ".strlen("Hello from VOsaka!")."\r\n";
            $response .= "\r\n";
...
function main(): Generator
{
    ...
    $listener = yield from TCPListener::bind("0.0.0.0:8099", [
        'accept_timeout' => 0.1,      // 100ms accept timeout
    ...
    echo "Max connections: ".$listener->getOptions()['max_connections']."\n";
    ...
    while (! $listener->isClosed()) {
        try {
            ...
            $connectionCount++;
            $currentConnections = $listener->getCurrentConnections();
            echo "New client accepted. Total connections: $currentConnections\n";
