...
function basicOperations(): Generator
{
    ...
    yield from Folder::createDir($testDir);

    // Create some subdirectories
    yield from Folder::createDir($testDir . "/subdir1");
    yield from Folder::createDir($testDir . "/subdir2/nested");
...
function walkDirectory(string $path): Generator
{
    ...
    $walker = Folder::walkDir($path, 2); // Max depth 2
...
function copyDirectory(string $source, string $destination): Generator
{
    ...
    $copiedFiles = yield from Folder::copyDir($source, $destination);
...
function tempDirectoryExample(): Generator
{
    ...
    $tempDir = yield from Folder::createTempDir("example_");
    ...
    yield from Folder::createDir($tempDir . "/temp_subdir");
...
function lockingExample(string $path): Generator
{
    ...
    try {
        ...
        $lockHandle = yield from Folder::lockDir($path, 5.0); // 5 second timeout
...
function metadataExample(string $path): Generator
{
    ...
    $metadata = yield from Folder::metadata($path);
...
function findFilesExample(string $path): Generator
{
    ...
    $finder = Folder::find($path, "*.txt", true);
...
function calculateSizeExample(string $path): Generator
{
    ...
    $size = yield from Folder::calculateSize($path);
...
function watchingExample(string $path): Generator
{
    ...
    $watcher = Folder::watchDir($path, 0.5); // Poll every 500ms
...
function main(): Generator
{
    ...
    // Cleanup
    echo "=== Cleanup ===\n";
    yield from Folder::removeDir($testDir);
    yield from Folder::removeDir($copyDir);
...
function concurrentExample(): Generator
{
    ...
    for ($i = 1; $i <= 3; $i++) {
        $tasks[] = function () use ($i): Generator {
            ...
            yield from Folder::createDir($dir);
    ...
    $sizeTasks = array_map(function ($dir) {
        return function () use ($dir): Generator {
            $size = yield from Folder::calculateSize($dir);
            return [$dir, $size];
        };
    }, $results);
    ...
    $cleanupTasks = array_map(function ($dir) {
        return function () use ($dir): Generator {
            yield from Folder::removeDir($dir);
            return basename($dir);
        };
    }, $results);
...
function errorHandlingExample(): Generator
{
    ...
    try {
        ...
        $reader = Folder::readDir("/non/existent/directory");
        foreach ($reader as $entry) {
            // This won't execute
        }
    } catch (Exception $e) {
